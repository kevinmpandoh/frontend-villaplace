...
import SidebarProfileDashboard from "./Sidebar/SidebarProfileDashboard";
import ChangePassword from "./Change-password/ChangePasswordOwner";
import ProfileDashboard from "./Profile/ProfileDashboard";
import EditProfileDashboard from "./Profile/EditProfileModalDashboard";
          ...
          >
            ...
            <div className="h-12 w-12 rounded-full border">
              <Image
                width={112}
                height={112}
                src={`http://localhost:8000/images/owner-profile/${data.data.foto_profile}`}
        ...
        <Modal title="Pengaturan" onClose={() => setModalOpen(false)}>
          <section className="flex-col h-full  flex xl:flex-row max-w-screen-xl mx-auto px-4">
            <SidebarProfileDashboard
              ...
              role={role}
            />
            <main className="xl:w-[80%] w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              {menuOpen === "profile" && (
                <ProfileDashboard
                  data={adminData || mitraData}
                  setMenuOpen={setMenuOpen}
              ...
              {menuOpen === "changePassword" && role !== "admin" && (
                <ChangePassword setMenuOpen={setMenuOpen} />
              )}
              {menuOpen === "changePassword" && role === "admin" && (
                <ChangePasswordAdmin setMenuOpen={setMenuOpen} />
              )}

              {menuOpen === "editProfile" && role !== "admin" && (
                <EditProfileDashboard
                  setMenuOpen={setMenuOpen}
                  data={mitraData}
                  handleSubmitMitra={handleUpdateMitra}
                />
              )}

              {menuOpen === "editProfile" && role === "admin" && (
                <EditProfileDashboardAdmin
                  setMenuOpen={setMenuOpen}
                  data={adminData}
                  handleSubmitAdmin={handleUpdateAdmin}
                />
              )}
            </main>
          </section>
